...
🔗 <a href="${shop.link}">Перейти</a>`)
    ...
    const secondHandsList = secondHands
        .map((shop) => `
♻ <b>${shop.name}</b>
📍 <i>${shop.address}</i>
🔗 <a href="${shop.link}">Перейти</a>
${shop.note ? `💡 <i>${shop.note}</i>` : ""}`)
        .join("\n\n");
    await ctx.reply(secondHandsList, {
        parse_mode: "HTML",
        link_preview_options: { is_disabled: true },
    });
})
...
${shop.note ? `💡 <i>${shop.note}</i>` : ""}`)
    ...
    const tireServicesList = tireServices
        .map((service) => `
🔧 <b>${service.name}</b>
📍 <i>${service.address}</i>
...
${service.note ? `💡 <i>${service.note}</i>` : ""}`)
        .join("\n\n");
    await ctx.reply(tireServicesList, {
        parse_mode: "HTML",
        link_preview_options: { is_disabled: true },
    });
})
...
${place.note ? `ℹ️ <i>${place.note}</i>` : ""}`)
        .join("\n\n");
    await ctx.reply(fastFoodPlacesList, {
        parse_mode: "HTML",
        link_preview_options: { is_disabled: true },
    });
})
