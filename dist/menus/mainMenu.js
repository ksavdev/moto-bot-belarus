...
${shop.note ? `ℹ️ <i>${shop.note}</i>` : ''}`).join("\n\n");
    await ctx.reply(`${shopListEu}\n\n🚚 Если знаете, кто может доставить из Европы в Беларусь, пишите, добавим информацию.`, { parse_mode: "HTML", link_preview_options: { is_disabled: true } });
    await ctx.menu.close();
})
    .row()
...
👤 <b>${service.username}</b>: ${service.services}`).join("\n");
